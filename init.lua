...
vim.g.copilot_no_tab_map = true -- Disable Copilot key mappings
...
vim.keymap.set('n', '<S-Up>', '<C-w>=', { silent = true, noremap = true })
...
vim.keymap.set('i', '<C-l>', '<Right>', { desc = 'Move right in insert mode' }) -- [[ Basic Autocommands ]]
...
vim.keymap.set('n', '<leader>p', function()
  ...
  for i, buf in ipairs(buffers) do
    if buf.bufnr == current then
      previous = buffers[i - 1] and buffers[i - 1].bufnr or buffers[#buffers].bufnr
      break
    end
  end
  if previous then
...
vim.keymap.set('n', '<leader>n', function()
  ...
  for i, buf in ipairs(buffers) do
    if buf.bufnr == current then
      next_buf = buffers[i + 1] and buffers[i + 1].bufnr or buffers[1].bufnr
      break
    end
  end
  if next_buf then
...
require('lazy').setup({
  ...
  { -- Autoformat
    ...
    opts = {
      ...
      format_on_save = function(bufnr)
        ...
        -- Disable formatting if the global or buffer-local variable is set
  ...
  { -- Autocompletion
    ...
    config = function()
      ...
      require('custom.snippets').setup() -- Add this line
